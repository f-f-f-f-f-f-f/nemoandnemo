...
Please report this to https://github.com/markedjs/marked.`, e) {
  ...
  var STATIC_DATA = {
    ...
    series: {
      all: "All Series",
      papaLouie: "Papa Louie",
      bloonsTowerDefense: "Bloons Tower Defense",
      ducklife: "DuckLife",
      fireboyWatergirl: "Fireboy and Watergirl"
    }
  };
  ...
  {
    const field = document.querySelector("#genre-search");
    for (const [key, genre] of Object.entries(STATIC_DATA.genres)) {
      const li = document.createElement("li");
      const a = document.createElement("a");
      a.textContent = genre;
      a.dataset.noNavigate = "";
      a.addEventListener("click", () => {
        {
          if (key === "all") {
            const els2 = Array.from(special.children);
            Array.from(els2).forEach((el) => el.style.display = "revert");
            return;
          }
          const selector = `[data-category="${key}"]`;
          const els = Array.from(special.querySelectorAll(selector));
          els.forEach((el) => el.style.display = "revert");
          const remaining = Array.from(
            Array.from(special.children).filter(
              (el) => !els.includes(el)
            )
          );
          remaining.forEach((el) => el.style.display = "none");
        }
      });
      li.appendChild(a);
  ...
  {
    const field = document.querySelector("#series-search");
    for (const [key, series] of Object.entries(STATIC_DATA.series)) {
      const li = document.createElement("li");
      const a = document.createElement("a");
